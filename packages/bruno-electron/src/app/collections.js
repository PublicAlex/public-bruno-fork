...
const openCollection = async (win, watcher, collectionPath, options = {}) => {
  ...
  try {
    ...
    // Auto-pull silencioso al abrir la colección. Conserva cambios locales no commiteados
    // y solo falla silenciosamente si hay conflictos, sin conexión, o sin remoto.
    silentPullGitChanges(collectionPath)
      ...
      .catch((err) => {
        console.warn(`[git auto-pull] Error en colección "${collectionPath}":`, err.message);
